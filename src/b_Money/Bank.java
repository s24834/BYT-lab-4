...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (getAccountlist().containsKey(accountid)) {
			throw new AccountExistsException();
		}
		else { // BŁĄD:  brakuje instrukcji dodającej konto do listy accountlist
			// accountlist.get(accountid);
			getAccountlist().put(accountid, new Account(accountid, currency));
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) { // BŁĄD: błędny warunek getAccountlist().containsKey(accountid
...
// BYŁ BŁĄD: wywoływano account.deposit(money) zamiast account.withdraw(money)
	...
	public void transfer(String fromaccount, Bank tobank, String toaccount, Money amount) throws AccountDoesNotExistException {
		if (!getAccountlist().containsKey(fromaccount) || !tobank.getAccountlist().containsKey(toaccount)) {
			throw new AccountDoesNotExistException();
		}
		else {
			getAccountlist().get(fromaccount).withdraw(amount);
			tobank.getAccountlist().get(toaccount).deposit(amount);
		}		
	}
	...
	/**
     * Get the account list of this bank
